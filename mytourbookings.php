...
$sessionid=session_id();
 $user_idquery=mysqli_query($dbConn,"SELECT active_user_id FROM activeusers WHERE session_id='$sessionid'");
    ...
    $packagedetailsquery="SELECT * FROM booking_packages AS bp,packages AS p,seats_available AS s WHERE bp.user_id=$user_id && bp.bookingp_id=p.package_id && bp.booking_seats_id=s.seats_id && (bp.status='booked' || bp.status='halfcancelled') ";
          ...
          <div id="content_outer">
            <div id="content">
              ...
              <div class="content_section">
                   <h1>MyTour Bookings  </h1>
                      <form action="viewcancelledtourbooking.php">
              ...
              <div class="content_section">
                ...
                if(mysqli_num_rows($packagedetails)==0)
                {
                  ...
                  while($row=mysqli_fetch_assoc($packagedetails))
                  {
                      ...
                      $customersquery=mysqli_query($dbConn,"SELECT * FROM bookingp_customers WHERE booking_cid=$booking_cid && status='booked' ");
                      while($row1=mysqli_fetch_assoc($customersquery))
                      {
                           ...
                           echo '<form method="post" action="canceltourbooking.php">
                                    <input type="hidden" name="booking_packages_id" value="'.$booking_cid.'" >
